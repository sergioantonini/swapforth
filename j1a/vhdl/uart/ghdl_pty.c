...
#define GHDL_UART_SYMLINK "/tmp/ghdl_uart"
int ptyf = -1;
...
int ghdl_pty_open(void)
{
  ...
  unlink(GHDL_UART_SYMLINK);
  if(symlink(ptsname(ptyf),GHDL_UART_SYMLINK)<0){
    perror("I can't create the /tmp/ghdl_uart symlink to the pseudoterminal\n");
    return -1;
  }
  return 0;
}
